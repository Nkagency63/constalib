...
const AccidentLocationMap = ({ lat, lng, address }: AccidentLocationMapProps) => {
  ...
  useEffect(() => {
    ...
    try {
      const newMap = L.map('accident-location-map').setView([lat, lng], 18);
  ...
  if (isLoading) {
    return (
      <div className="rounded-lg overflow-hidden shadow-lg bg-gray-100 animate-pulse" style={{ height: '500px' }}>
        <div className="h-full w-full flex items-center justify-center">
