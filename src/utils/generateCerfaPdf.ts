...
/**
 * Génère un PDF Cerfa à partir des données du constat amiable
 * @param formData Données du formulaire de constat
 * @param schemeImageDataUrl Image du schéma d'accident (optionnel)
 * @param signatures Signatures des parties (optionnel)
...
export async function generateCerfaPdf(
  ...
  try {
    ...
    // 1. Ajouter les informations de base de l'accident
    if (formData.date) {
    ...
    // 2. Ajouter les informations des véhicules
    ...
    // 3. Ajouter les informations des conducteurs
    ...
    // 4. Ajouter les circonstances si disponibles
    if (formData.vehicleACircumstances?.length) {
      formData.vehicleACircumstances.forEach(circ => {
        ...
        const circIdStr = typeof circ === 'string' ? circ : (circ as any).id;
    ...
    // 5. Ajouter le schéma s'il est fourni
    if (schemeImageDataUrl) {
      try {
        ...
        // Dessiner l'image sur la page dans la section du schéma
    ...
    // 6. Ajouter les signatures si fournies
    if (signatures) {
 ...
 */
export async function downloadCerfaPdf(pdfUrl: string, fileName: string = 'constat-amiable.pdf'): Promise<boolean> {
  try {
